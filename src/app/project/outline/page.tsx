...
import { addDays } from "date-fns"; // date-fns에서 addDays 함수 사용
...
export default function ProjectOutlinePage() {
  const [summaryValue, setSummaryValue] = useState(""); // 사회문제 요약 입력 상태
  const [titleValue, setTitleValue] = useState(""); // 프로젝트 제목 입력 상태
  ...
  const handleStartDateChange = (date: Date | null) => {
    setStartDate(date);
    if (date) {
      setEndDate(addDays(date, 100)); // 시작 날짜로부터 100일 후 날짜로 종료 날짜 설정
    }
  };
  ...
  return (
    <div className="flex w-full flex-col justify-center items-center pt-[80px]">
      <div className="flex flex-col items-start gap-[0.9375rem] pt-[2.94rem] pb-[12rem]">
        ...
        <div className="flex flex-col justify-center items-center w-[62.5rem] p-[1.875rem] gap-[4.9375rem] rounded-[1rem] bg-[#FEFEFE]">
          ...
          <div className="flex flex-col items-start h-[10.8125rem] gap-[2rem] self-stretch">
            ...
            <Dropdown
              name="target"
              placeholder="대상 선택"  // Placeholder 값 설정
          ...
          <div className="flex flex-col items-start h-[10.8125rem] gap-[2rem] self-stretch">
            ...
            <div className="flex items-center gap-[0.5rem]">
              ...
              />
              <div className="text-[1.125rem] text-[#4E525C] font-normal leading-[1.6875rem] font-pretendard">~</div> {/* 물결 모양 추가 */}
              ...
              <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer rounded-md text-center text-[#4E525C] text-[0.75rem] font-normal leading-[1.125rem]">
                {representativeImage ? (
                  <Image
                    ...
                    layout="fill"
                    objectFit="contain" // 이미지가 짤리지 않고 전체가 보이도록 설정
                    alt="대표 사진"
          ...
          <div className="flex pl-[51.8125rem] justify-end items-center self-stretch">
            <div className="flex items-start gap-[0.625rem]">
              <button className="flex w-[6.9375rem] h-[3.5rem] min-w-[6rem] px-[1.75rem] py-[1.125rem] justify-center items-center gap-[0.625rem] rounded-[0.5rem] bg-[#776BFF]">
